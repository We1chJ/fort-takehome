...
The **silent case** is a first-class demo, not a gap. Load *an ordinary leg day*, run it to
the end, and pull the sheet: the system correctly reports that nothing departed from how
this lifter's sessions usually go.
...
```
session source            generator (authored) | MM-Fit replay (not built)
      │                   both would emit the same schema
      ▼
...
Eighteen sessions over thirty days, on a four-day rotation with a deliberately thin patch
around week three. Time is kept as "days ago" rather than calendar dates — it sidesteps
timezones and keeps the aggregation pure, which is what makes it testable.
...
| [MM-Fit](https://mmfit.github.io/) | MIT | **Not built.** The intended real-replay source: wrist smartwatch accel/gyro at 100 Hz plus HR at 1 Hz, rep-labelled. It would slot in behind `SessionEvent` with no engine changes. |

Session data is generated by `src/session/generator.ts` from a deterministic PRNG, so
scenarios replay identically and tests are stable.
